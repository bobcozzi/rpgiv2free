...
- **PLIST/PARM → DCL-PI conversion**: Fixed-format `*ENTRY PLIST` + `PARM` groups are now converted to a free-format `dcl-pi *n; ... end-pi;` procedure-interface block. Parameter types are inferred from the inline length/decimal columns; when no inline length is present the parameter is emitted as `like(name)` to defer to the D-spec definition.
- **Named PLIST → DCL-PR conversion**: A named `PLIST` + `PARM` group (used as a parameter list for `CALL`) is converted to a `dcl-pr name; ... end-pr;` prototype block, placed in the definition section of the output.
- **CALL/CALLB → free-format call + DCL-PR**: `CALL` and `CALLB` statements are converted to a free-format `pgmname(arg1 : arg2);` call statement. A corresponding `dcl-pr pgmname extpgm('PGMNAME'); ... end-pr;` prototype is generated and placed in the definition area. Inline PARM lists and externally defined named PLISTs are both supported.
- **New `src/refactor.ts` module**: Contains `collectPLIST()` (pre-pass scanner mirroring the `collectKLIST` pattern), `buildPIBlock()`, `buildPRBlock()`, `getFreeCallLines()`, and supporting helpers. Exported via the `src/opcodes.ts` barrel.
- **TypeScript configuration fixes**: Added `"dom"` to `tsconfig.json` `lib` and `"node"` to `types`; installed `@types/node`. Resolves pre-existing `console`, `NodeJS`, `path`, `setTimeout`, and `clearTimeout` compile errors.
- **Syntax highlighter disabled**: The experimental built-in syntax highlighter (`registerSyntaxHighlighting`) has been temporarily disabled. When active it conflicted with the Code for IBM i extension's fixed-format RPG IV syntax highlighting, preventing it from colouring source correctly. The code remains in place and can be re-enabled when the conflict is resolved.

## [1.12.24] - 2026-04-20
- **Multi-CCSID national variant identifier support (CCSID 273, 280, 297)**: Extends the CCSID 277 support introduced in 1.12.23 to three additional European IBM i code pages. On German (CCSID 273) and Italian (CCSID 280) systems, the EBCDIC byte position `X'7C'` (equivalent to `@` in CCSID 37) maps to `§` (U+00A7, section sign). On French (CCSID 297) and Italian (CCSID 280) systems, the byte position `X'7B'` (equivalent to `#` in CCSID 37) maps to `£` (U+00A3, pound sign). On French (CCSID 297) systems, `X'7C'` maps to `à` (U+00E0). A German programmer who types `§Betrag` is using the exact same EBCDIC byte (X'7C') that a US developer types as `@`. All four new Unicode code points — `§` (U+00A7), `£` (U+00A3), `à` (U+00E0), `À` (U+00C0) — are now recognised as valid RPG IV identifier characters by the tokenizer, preventing field names such as `§Betrag`, `£TaxAmt`, or `àFeld` from being incorrectly split during fixed-to-free conversion. The `RPG_NAME_VARIANT_CHARS` constant and all identifier regex patterns in `formatRPGIV.ts`, `rpgedit.ts`, and `defntools.ts` have been updated. CCSID 37 identifiers (`#`, `$`, `@`) and CCSID 277 identifiers (`Æ`, `Ø`, `æ`, `ø`) are unaffected.

## [1.12.23] - 2026-04-20
- **CCSID 277 (Danish/Norwegian) identifier support**: On Danish/Norwegian IBM i systems (CCSID 277), the characters `Ø` and `Æ` occupy the same EBCDIC byte positions as `#` and `@` on US (CCSID 37) systems. They are the standard RPG name prefix characters for those users — a Danish developer who types `Ø` is using the exact same EBCDIC byte that a US developer types as `#`. The converter now correctly recognizes `Ø` (U+00D8), `Æ` (U+00C6), and their lowercase forms `ø` (U+00F8) and `æ` (U+00E6) as valid RPG IV identifier characters. Previously, field names such as `ØrendsNr` or `Æfelt` were incorrectly split by the tokenizer because these Unicode code points were absent from all identifier character classes. All relevant regex patterns in `formatRPGIV.ts`, `rpgedit.ts`, and `defntools.ts` have been updated. A new exported constant `RPG_NAME_VARIANT_CHARS` consolidates these code points for future CCSID expansion (e.g., German CCSID 273, French CCSID 297). CCSID 37 identifiers (`#`, `$`, `@`) are unaffected.
...
- Fixed a formatting issue where, when the statement length hit the line length (for example, exactly 80 bytes), the formatter would wrap the closing semicolon onto a new line by itself, or in some cases include the right parenthesis if it were the last character on the line.
