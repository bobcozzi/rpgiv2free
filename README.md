...
This extension is **NOT a refactoring or modernization tool**. Its only purpose is to convert RPG IV fixed-format statements to free format RPG IV syntax. It `does not convert` legacy RPG II Cycle code.
...
- **Multi-Line, Multi-Statement and Compound Statement Support**:
  A `line of code` is one line in the source file. An `RPG statement`, such as a D spec or Calc spec can span multiple lines of code, for example when a File or Definition spec have additional keywords on additional lines of code, or you have a Calc spec with IF/ELSEIF/EVAL/EVALR whose statement spans multiple lines of code. There are also so called compound RPG opcodes such as the legacy IFEQ/OREQ/ANDEQ/WHENxx and CASxx that can span multiple lines. This extension allows you to place the cursor on any part of the statement and right-click and convert the entire statement. The only exception is a set of CASxx opcodes. Prior to supporting conditioning indicators, we allowed it to be treated like the IFxx/ORxx/ANDxx/WHENxx opcodes. Today each CASxx is treated independently to support the unlikely situtional of it also having conditioning indicators. However, you may select the entire CASxx/ENDCS block and convert it at one time, it should work the way you expect it.
